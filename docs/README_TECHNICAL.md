...
- **Persistence**: Metadata (titles, IDs, due dates) is cached in the local SQLite database (`materials` table). Rich content and attachments are indexed into the Memsapien vector store.
...
- **Persistence**: Identified insights are stored in Memsapien. Highly personal or actionable items are synthesized into the user's Long-Term Memory (Digital Twin).
...
- **Persistence**: Extracted text is indexed into Memsapien for RAG retrieval.
...
- **Persistence**: Similar to Classroom, metadata is stored locally while core resource content is indexed in Memsapien.

### 1.5 Direct Chat Uploads
- **Scope**: User-uploaded PDFs, text files, and images.
- **Handling**: Text is extracted locally.
- **Persistence**: Uploaded text content is immediately indexed into Memsapien under the category `chat_attachment`, tagged with the specific `thread_id`.
...
- **Storage**: These insights are formatted into a "User Profile" document in Memsapien, which serves as the persistent "Digital Twin" identity.
...
- **Memsapien Costs**: Tracking token-based indexing and search overhead.
